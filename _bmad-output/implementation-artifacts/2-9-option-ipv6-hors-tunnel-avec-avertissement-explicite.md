...
- [ ] **Task 1 — Schéma config TOML** (AC: #1, #6)
  - [ ] Ajouter section `[tunnel]` avec champ `allow_ipv6_leak: bool` (default `false`) dans `internal/config`
  - [ ] Mettre à jour `config.example.toml` avec commentaire explicite sur risque de fuite
  - [ ] Tests unitaires : parsing, valeur par défaut, round-trip save/load
- [ ] **Task 2 — API firewall IPv6 (Linux nftables)** (AC: #1, #3, #4)
  - [ ] Dans `internal/firewall/linux` (nouveau package), ajouter fonction `SetIPv6Policy(allowLeak bool)` qui génère/applique le ruleset nftables
  - [ ] Ruleset `allowLeak=false` : `ip6 daddr != <relay-ipv6-if-any> drop` sur chain output, ET drop sur chaîne forward pour toutes destinations IPv6
  - [ ] Ruleset `allowLeak=true` : retirer les règles de drop IPv6, conserver seulement les règles IPv4 kill switch
  - [ ] Idempotence : `nft flush` + ré-application atomique via transaction
  - [ ] Tests d'intégration (build tag `linux`) vérifiant état nftables post-appel
- [ ] **Task 3 — API firewall IPv6 (Windows WFP)** (AC: #1, #3, #4)
  - [ ] Dans `internal/firewall/windows` (nouveau package), ajouter filtres WFP pour `FWPM_LAYER_ALE_AUTH_CONNECT_V6` bloquant tous connects IPv6
  - [ ] Lorsque `allowLeak=true` : retirer ces filtres v6 mais conserver filtres v4
  - [ ] Persistence des filtres avec `FWPM_FILTER_FLAG_PERSISTENT` (survit au crash service — NFR9b)
  - [ ] Cleanup propre au stop service (retirer filtres via GUID)
- [ ] **Task 4 — Interface plateforme et intégration service** (AC: #3, #6)
  - [ ] Interface Go `firewall.Manager` avec `SetIPv6Policy(bool) error` implémentée par build tags linux/windows
  - [ ] Au démarrage service : lire config TOML puis appeler `SetIPv6Policy(cfg.Tunnel.AllowIPv6Leak)` avant d'activer le tunnel
  - [ ] Ordre : kill switch IPv4 appliqué → policy IPv6 appliquée → tunnel monté (évite fenêtre de fuite)
- [ ] **Task 5 — IPC service ↔ UI** (AC: #3)
  - [ ] Ajouter commandes IPC `GetAllowIPv6Leak() bool` et `SetAllowIPv6Leak(bool)` dans `internal/ipchandler`
  - [ ] `SetAllowIPv6Leak` : update firewall + save config TOML atomiquement (une seule transaction, rollback si échec firewall)
  - [ ] Tests IPC : roundtrip true/false, gestion erreur firewall
- [ ] **Task 6 — UI Paramètres : checkbox + modale** (AC: #2, #3, #4)
  - [ ] Dans `frontend/` (webview vanilla HTML/CSS/JS) : ajouter section "Avancé" dans le panel Paramètres sous la section WebRTC
  - [ ] Checkbox `[ ] Autoriser IPv6 hors tunnel` avec description courte sous la case
  - [ ] Modale d'avertissement avec texte exact spécifié dans AC2, bouton `Continuer` en rouge `#d42b2b`, `Annuler` par défaut avec focus initial
  - [ ] État optimiste interdit : checkbox coche seulement après réponse IPC OK
  - [ ] Décochage sans modale (AC4)
- [ ] **Task 7 — Indicateur visuel "IPv6 non protégé"** (AC: #3)
  - [ ] Badge orange dans titlebar (à côté du V) visible en permanence si `allow_ipv6_leak=true`
  - [ ] Ligne d'état dans panel Paramètres : "⚠ IPv6 exposé hors tunnel" en orange `#e89332`
  - [ ] Tooltip au hover : "Votre IP IPv6 réelle est visible pour les services IPv6"
- [ ] **Task 8 — Tests e2e** (AC: #1, #5)
  - [ ] Test Linux (Ubuntu 24.04) : default → `ping6 -c1 2606:4700:4700::1111` échoue ; after enable → succeed
  - [ ] Test Windows 11 : équivalent avec `Test-NetConnection -IPv6`
  - [ ] Test capture : `tshark -f "ip6"` sur interface physique — 0 paquet en mode default, paquets visibles en mode opt-out
  - [ ] Test failover : `allow_ipv6_leak=true`, forcer reconnexion relais — IPv6 reste autorisé pendant failover
