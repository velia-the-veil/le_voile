...
- [ ] **Task 1 : Créer le package `internal/firewall/` avec interface cross-platform** (AC: #1, #5)
  - [ ] Créer `internal/firewall/firewall.go` : interface `Firewall` avec `Activate(ctx context.Context, relayIP net.IP, tunName string) error`, `Deactivate(ctx context.Context) error`, `IsActive(ctx context.Context) (bool, error)`
  - [ ] Créer `internal/firewall/doc.go` décrivant le contrat (règle de base : Activate AVANT tunnel, Deactivate APRÈS fermeture tunnel ; jamais de Deactivate sans reconnect immédiat — cf architecture.md § Enforcement Guidelines)
  - [ ] Fournir un constructeur `New()` avec build tags qui retourne l'impl adéquate par OS
  - [ ] Implémenter un stub `firewall_stub.go` (build tag `!linux && !windows`) pour permettre la compilation sur macOS/dev

- [ ] **Task 2 : Implémenter WFP Windows** (AC: #1, #2, #5)
  - [ ] Créer `internal/firewall/firewall_windows.go` (build tag `windows`)
  - [ ] Définir des GUID constants stables pour le provider Le Voile et le sublayer : `providerKey` (ex: `{4e7c2b4f-...}`) et `sublayerKey`. Weight sublayer : max (0xFFFF) pour précéder Windows Firewall
  ...
  - [ ] Poser les filters suivants dans une transaction WFP atomique (tous sous notre sublayer, action BLOCK sauf mention contraire) :
  ...
  - [ ] Chronométrer `Activate()` de bout en bout, logger la durée, et retourner erreur si > 100ms (NFR15)
  - [ ] Implémenter `Deactivate()` : ouvrir transaction, énumérer filters par `providerKey`, delete tous, delete sublayer, delete provider, commit. Idempotent : si provider absent → no-op
  - [ ] Implémenter `IsActive()` : ouvrir engine read-only, énumérer filters par `providerKey`, retourner `count > 0`

- [ ] **Task 3 : Détection d'élévation et pré-checks Windows** (AC: #1)
  - [ ] Avant le premier `Activate()`, vérifier que le process tourne en `LocalSystem` via le package existant `internal/elevation/`. Si non → retourner erreur claire "WFP requires LocalSystem (installer via kardianos/service)" et refuser le démarrage tunnel
  - [ ] Vérifier disponibilité de `fwpuclnt.dll` (présent nativement Windows 7+). Échec → erreur claire

- [ ] **Task 4 : Crash-recovery au démarrage service** (AC: #3)
  - [ ] Dans l'init service Windows (`internal/service/`), avant le premier `Activate()`, appeler `firewall.CleanupOrphans()` (nouvelle méthode publique du package) qui ouvre une session WFP, énumère les filters par `providerKey` et les supprime si présents (indice d'un crash précédent)
  - [ ] Chronométrer le cleanup, logger la durée, assert < 5s (NFR17). Si > 5s → log WARNING Event Log mais continuer
  - [ ] Écrire une entrée Event Log niveau INFO : "WFP orphan filters cleaned: {count}" (sans données utilisateur, NFR22a)

- [ ] **Task 5 : Watchdog altération tiers** (AC: #4)
  - [ ] Créer une goroutine watchdog dans `internal/firewall/watchdog_windows.go` démarrée par `Activate()` avec `context.Context`, polling toutes les 3s
  - [ ] Chaque tick : énumérer filters par `providerKey`, comparer le count à la valeur `expectedFilterCount` capturée à la fin de `Activate()`. Si inférieur → détection altération
  - [ ] En cas d'altération : envoyer un event sur un channel `FirewallAlteredCh` observé par `internal/service/`, lequel (a) remonte une alerte via IPC handler vers UI ("Règles firewall altérées par tiers — protection compromise"), (b) écrit Event Log WARNING, (c) déclenche `Deactivate() → Activate()` pour restaurer les règles
  - [ ] Arrêter la goroutine proprement via `context.Done()` à `Deactivate()`

- [ ] **Task 6 : Intégration dans l'orchestration service** (AC: #1, #2, #3)
  - [ ] Modifier `internal/service/` pour respecter l'ordre strict Connect : `elevation.Check() → tun.New() → routing.Setup() → firewall.Activate(relayIP, tunName) → tunnel.Connect()` et l'ordre strict Disconnect : `tunnel.Disconnect() → firewall.Deactivate() → routing.Teardown() → tun.Close()` (cf architecture.md § Enforcement Guidelines)
  - [ ] Au démarrage du service : appel préalable `firewall.CleanupOrphans()`
  - [ ] Pendant failover/reconnect : garder le firewall actif. Ne pas appeler `Deactivate()` sauf shutdown définitif (cf NFR9b, anti-pattern "Désactiver le firewall sans reconnect immédiat")

- [ ] **Task 7 : Event Log Windows** (AC: #3, #4)
  - [ ] Utiliser `golang.org/x/sys/windows/svc/eventlog` (déjà disponible via dep `kardianos/service`) pour écrire dans la source "LeVoile"
  - [ ] Messages : "WFP activated in {ms}ms", "WFP deactivated", "WFP orphans cleaned: {n}", "WFP altered by third party — restoring", etc. Aucune donnée utilisateur (pas de relayIP, pas de tunName)

- [ ] **Task 8 : Config TOML** (AC: #1)
  - [ ] Ajouter section `[firewall]` dans `internal/config/` avec clé `enable_killswitch = true` (default). Si `false`, `Activate()` est no-op (mode dégradé explicite, cf Story 5.9)

- [ ] **Task 9 : Tests** (AC: #1, #2, #3, #4, #5)
  - [ ] Créer `internal/firewall/firewall_windows_test.go` — tests d'intégration sous Windows uniquement (build tag `windows` + tag `integration`), à exécuter en LocalSystem (runner CI Windows avec élévation ou mock WFP)
  - [ ] Test TC-2.7.1 : `Activate` → `IsActive()` == true, durée < 100ms
  - [ ] Test TC-2.7.2 : `Activate` puis tentative de connexion TCP vers un hôte externe hors TUN → bloqué (ECONNREFUSED/timeout)
  - [ ] Test TC-2.7.3 : `Activate` puis tentative de connexion UDP vers `relayIP:443` → autorisée
  - [ ] Test TC-2.7.4 : `Activate` → simuler crash (kill goroutine sans Deactivate) → filters persistent → nouveau process appelle `CleanupOrphans` → state clean, durée < 5s
  - [ ] Test TC-2.7.5 : `Deactivate` idempotent (appel ×2 sans erreur)
  - [ ] Test TC-2.7.6 : watchdog détecte suppression manuelle d'un filter (via `FwpmFilterDeleteById0` externe) en < 5s et déclenche alerte
  - [ ] Test TC-2.7.7 : DHCP renew fonctionne avec firewall actif (UDP 67/68 autorisé)
