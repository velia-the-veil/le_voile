...
func TestTunnelHandler_TolerantOnForwarderError(t *testing.T) {
	...
	// R-T8 round 7 (2026-05-11) — message changé de "dropping packet" à
	// "drop pkt" pour compacter le format enrichi avec err/ipver/proto/len.
	if !strings.Contains(logBuf.String(), "drop pkt") {
		t.Errorf("log missing 'drop pkt' note: %q", logBuf.String())
	}
}
