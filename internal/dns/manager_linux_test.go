...
func TestParseAllResolvectlInterfaces(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "single",
			input:  "Link 2 (eth0): 192.168.1.1\n",
			expect: []string{"eth0"},
		},
		{
			name:   "multiple",
			input:  "Link 2 (eth0): 192.168.1.1\nLink 3 (wlan0): 8.8.8.8 8.8.4.4\n",
			expect: []string{"eth0", "wlan0"},
		},
		{
			name:   "empty",
			input:  "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := parseAllResolvectlInterfaces(tt.input)
			if len(got) != len(tt.expect) {
				t.Errorf("parseAllResolvectlInterfaces got %d interfaces, want %d", len(got), len(tt.expect))
				return
			}
			for i := range got {
				if got[i] != tt.expect[i] {
					t.Errorf("interface[%d] = %q, want %q", i, got[i], tt.expect[i])
				}
			}
		})
	}
