...
# Cleanup relay/deploy/install.sh.sig from any prior run so we can assert the
# before.hook really produced it this run (fix C7 defensive smoke).
rm -f relay/deploy/install.sh.sig
...
if [[ ! -s relay/deploy/install.sh.sig ]]; then
  fail "relay/deploy/install.sh.sig missing after goreleaser — before.hook regressed?"
fi
if ! verifypkg -pubkey "${PUB_B64}" relay/deploy/install.sh relay/deploy/install.sh.sig >/dev/null 2>&1; then
  fail "relay/deploy/install.sh.sig does not verify against ephemeral key"
fi
log "  ✓ relay/deploy/install.sh.sig produced and verifies"
